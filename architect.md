...
```text
          ...
          synthetic/generate_synthetic.py ► data/*.csv   (dev/test laps)
                      │
                      ▼
...
| `pitmind/segmentation.py` | Split a recording into laps; drop invalid / out-laps | `normalized_car_position` wrap + lap counter |
| `pitmind/corners.py` | Detect corners from telemetry only (track-agnostic) | cumulative heading change from x/y, curvature, brake zones |
| `pitmind/events.py` | Extract corner metrics: brake point, entry/apex/exit speed, throttle-on point | signal logic (brake/throttle/speed transitions) |
| `pitmind/features.py` | Build the per-lap-per-corner feature table | aggregation |
...
3. `corners` → list of corners with index ranges (entry → apex → exit).
...
- **Tests:** pytest against synthetic data with known injected mistakes.

## Hard Architectural Rules

1. **No per-track geometry.** Corner detection is derived from telemetry (x/y, heading change,
   `normalized_car_position`). Any ACC F1 circuit works without config.
